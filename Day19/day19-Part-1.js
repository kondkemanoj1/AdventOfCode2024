...
const desiredDesigns= desiredDesignsStr.split(/\r?\n/).map((str)=> str.trim());

// Function to check if a design is possible using available towel patterns
function isDesignPossible(patterns, design) {
  // Recursive helper function to check if the design can be constructed
  function canConstruct(remainingDesign, memo = {}) {
    if (remainingDesign === "") return true; // Design is fully constructed
    ...
    for (const pattern of patterns) {
      if (remainingDesign.startsWith(pattern)) {
        ...
        if (canConstruct(nextDesign, memo)) {
...
desiredDesigns.forEach((design) => {
  if (isDesignPossible(towelPatterns, design)) {
    possibleDesignsCount++;
  }
});

console.log(`Number of possible designs: Part 1 -  ${possibleDesignsCount}`);
